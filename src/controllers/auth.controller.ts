...
export const registerUser = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      // Non-admin users cannot create admin
      ...
      await RedisService.setEx(
        CacheKeys.userProfile(newUser.UserID),
        3600,
        newUser,
      );
...
export const loginUser = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    await RedisService.setEx(CacheKeys.userActivity(user.UserID), 86400, {
...
export const logoutUser = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (token) {
      ...
      if (expiresIn > 0) {
        await RedisService.setEx(CacheKeys.blacklistedToken(token), expiresIn, {
          blacklisted: true,
        });
      }
    }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!activity) {
        ...
        const userProfile = await RedisService.get(
