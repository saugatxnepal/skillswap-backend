...
export const setupSocket = (server: HttpServer) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("conversation:join", (conversationId: string) => {
      ...
      console.log(
        `[Socket] User ${user.fullName} joined conversation ${conversationId}`,
      );
    });
    ...
    socket.on("conversation:leave", (conversationId: string) => {
      ...
      console.log(
        `[Socket] User ${user.fullName} left conversation ${conversationId}`,
      );
    });
    ...
    socket.on("session:join", async (sessionId: string) => {
      ...
      if (sessionCheck.rows.length > 0) {
        ...
        console.log(
          `[Socket] User ${user.fullName} joined session ${sessionId}`,
        );
    ...
    socket.on(
      "message:read",
      async (data: { sessionId: string; messageIds: string[] }) => {
    ...
    socket.on("webrtc:join-room", ({ roomId, userId, userName }) => {
      socket.join(`webrtc:${roomId}`);

      // Store room info in socket
      (socket as any).webrtcRoomId = roomId;
      (socket as any).webrtcUserId = userId;
      (socket as any).webrtcUserName = userName;

      console.log(
        `[WebRTC] User ${userName} (${userId}) joined room ${roomId}`,
      );
        ...
        for (const socketId of room) {
          if (socketId !== socket.id) {
            ...
            if (participantSocket) {
              participants.push({
                userId: (participantSocket as any).webrtcUserId,
                userName: (participantSocket as any).webrtcUserName,
      ...
      socket.emit("webrtc:existing-participants", participants);
    });
    ...
    socket.on("webrtc:offer", ({ to, offer }) => {
      ...
      socket.to(to).emit("webrtc:offer", {
        from: socket.id,
        offer,
        fromUserId: (socket as any).webrtcUserId,
        fromUserName: (socket as any).webrtcUserName,
      });
    });
    ...
    // WebRTC: Screen Share Offer
    socket.on("webrtc:screen-offer", ({ to, offer }) => {
      console.log(`[WebRTC] Screen share offer from ${socket.id} to ${to}`);
      socket.to(to).emit("webrtc:screen-offer", {
        from: socket.id,
        offer,
      });
    });

    // WebRTC: Screen Share Answer
    socket.on("webrtc:screen-answer", ({ to, answer }) => {
      socket.to(to).emit("webrtc:screen-answer", {
        from: socket.id,
        answer,
      });
    });

    // WebRTC: Toggle Screen Share
    socket.on("webrtc:toggle-screen-share", ({ isSharing }) => {
      const roomId = (socket as any).webrtcRoomId;
      if (roomId) {
        socket.to(`webrtc:${roomId}`).emit("webrtc:screen-share-status", {
          userId: (socket as any).webrtcUserId,
          userName: (socket as any).webrtcUserName,
          isSharing,
        });
      }
    });
    ...
    // WebRTC: Toggle Audio
    socket.on("webrtc:toggle-audio", ({ isMuted }) => {
      const roomId = (socket as any).webrtcRoomId;
      if (roomId) {
        socket.to(`webrtc:${roomId}`).emit("webrtc:audio-status", {
          userId: (socket as any).webrtcUserId,
          userName: (socket as any).webrtcUserName,
          isMuted,
        });
    ...
    // WebRTC: Toggle Video
    socket.on("webrtc:toggle-video", ({ isOff }) => {
      const roomId = (socket as any).webrtcRoomId;
      if (roomId) {
        socket.to(`webrtc:${roomId}`).emit("webrtc:video-status", {
          userId: (socket as any).webrtcUserId,
          userName: (socket as any).webrtcUserName,
          isOff,
        });
    ...
    socket.on("webrtc:leave-room", () => {
      ...
      if (roomId) {
        ...
        (socket as any).webrtcRoomId = null;
      }
    });
    ...
    socket.on("message:delete", async (data: { messageId: string }) => {
      ...
      try {
        ...
        if (result.rows.length > 0) {
          const { SessionID, ConversationID } = result.rows[0];
          if (SessionID) {
            io.to(`session:${SessionID}`).emit("message:deleted", {
              messageId,
            });
          } else if (ConversationID) {
            const convResult = await query(
    ...
    socket.on("session:get-online-users", (sessionId: string, callback) => {
      ...
      if (room) {
        for (const socketId of room) {
          const socketUser = Array.from(onlineUsers.values()).find(
